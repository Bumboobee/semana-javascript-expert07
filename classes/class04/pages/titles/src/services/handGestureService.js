export default class HandGestureService {
    ...
    constructor({ fingerpose, handPoseDetection, handsVersion, knowGesture, gestureStrings }) {
    ...
    async * detectGestures(preditions) {
        for(const hand of preditions) {
            ...
            const othersGesturesIcon = this.#gestureStrings[result.name]

            yield { event: result.name, x, y, othersGesturesIcon }
            console.warn('detect', this.#gestureStrings[result.name])
        }
    }
